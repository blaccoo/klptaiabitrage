...
function checkPasswordStrength() {
  ...
  if ($("#reg_password").val().length < 6) {
    ...
    if (
      $("#reg_password").val().match(number) &&
      $("#reg_password").val().match(alphabets) &&
      $("#reg_password").val().match(special_characters)
    ) {
      $("#password-strength-status").removeClass();
...
function MsgCustomSuccess(msg, element, time) {
  var show =
    "<div class='row'><div class='notice-success'>" + msg + "</div></div>";
  setTimeout(function () {
    $(element).html("");
  }, time);
  $(element).html(show);
...
function MsgCustomDanger(msg, element, time) {
  var show =
    "<div class='row'><div class='notice-danger'>" + msg + "</div></div>";
  setTimeout(function () {
    $(element).html("");
  }, time);
  $(element).html(show);
...
function Msg(msg, color, dismiss, element, time) {
  if (dismiss == 1) {
    var show =
      "<div class='row1' id='alert_div'><div class='col-lg-12'><div class='alert " +
      color +
      " alert-dismissable'><button type='button' onclick='$(\"#alert_div\").toggle()' style='width:auto; padding:1px; height: 30px;left:5%; margin:0px' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>" +
      msg +
      "</div></div></div>";
  } else if (dismiss == 0) {
    var show =
      "<div class='row1'><div class='col-lg-12'><div class='alert " +
      color +
      "'>" +
      msg +
      "</div></div></div>";
...
function Msg2(msg, color, dismiss, element) {
  if (dismiss == 1) {
    var show =
      "<div class='row1' id='alert_div'><div class='col-lg-12'><div class='alert " +
      color +
      " alert-dismissable'><button type='button' class='close' onclick='$(\"#alert_div\").toggle()' data-dismiss='alert' aria-hidden='true'>&times;</button>" +
      msg +
      "</div></div></div>";
  } else if (dismiss == 0) {
    var show =
...
function Msg3(msg, color, dismiss, element) {
  if (dismiss == 1) {
    var show =
      "<div class='row1' id='alert_div'><div class='col-lg-12'><div class='alert " +
      color +
      " solid alert-dismissable'><button type='button' class='close' onclick='$(\"#alert_div\").toggle()' data-dismiss='alert' aria-hidden='true'>&times;</button>" +
      msg +
      "</div></div></div>";
  } else if (dismiss == 0) {
    var show =
